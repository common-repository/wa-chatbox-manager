	...
	class chatbox_manager_panel {
		...
		public function save_option() {
			...
			if ( $this->chatbox_manager_request('chatbox_manager_action') == 'New chatbox' ) {
				...
				if (isset($chatbox_manager_setting['chatbox_manager_chatboxes']) && is_array($chatbox_manager_setting['chatbox_manager_chatboxes'])) {
					$getlastChatbox = key(array_slice($chatbox_manager_setting['chatbox_manager_chatboxes'], -1, 1, true));
				} else {
					$getlastChatbox = 'chatbox0';
				}

				$lastChatbox = str_replace('chatbox', '', $getlastChatbox);
				$newChatbox = intval($lastChatbox) + 1;
							...
							echo $chatboxManagerForm->elementStart('div', $plugin_slug . 'tabs', FALSE );
								...
								echo $chatboxManagerForm->elementStart('div', $plugin_slug . 'header', FALSE );
									...
									echo $chatboxManagerForm->elementStart('div', FALSE, 'left plugin_description' );
										...
										echo $chatboxManagerForm->link('https://wordpress.org/support/plugin/wa-chatbox-manager/', FALSE, FALSE, '_blank', FALSE, esc_html__( ' - Support','chatbox-manager') );
										echo $chatboxManagerForm->link('https://wordpress.org/support/plugin/wa-chatbox-manager/reviews/', FALSE, FALSE, '_blank', FALSE, esc_html__( ' - Rate this plugin on WordPress.org','chatbox-manager') );
								...
								echo $chatboxManagerForm->elementStart('div', FALSE, $plugin_slug . 'box' );
									...
									echo $chatboxManagerForm->tableElementStart('tbody', FALSE, 'table-body');
										...
										echo $chatboxManagerForm->tableElementStart('tr', FALSE, 'upsell-row');
											...
											echo $chatboxManagerForm->tableElementStart('td', FALSE, FALSE);
											
												echo $chatboxManagerForm->tableElementStart('p', FALSE, FALSE);

													echo $chatboxManagerForm->element('span', FALSE, FALSE, esc_html__( 'Unlock a 50% discount on all plans now!', 'chatbox-manager' ) .'<br/>');
													echo $chatboxManagerForm->element('span', FALSE, FALSE, esc_html__( 'Use the limited-time coupon code! ', 'chatbox-manager' ));
													echo $chatboxManagerForm->element('strong', FALSE, FALSE, esc_html__( 'CHATBOXMANAGER50', 'chatbox-manager' ));

												echo $chatboxManagerForm->tableElementEnd('p');
										...
										if (!isset($_REQUEST['chatboxID'])) {
											...
											echo $chatboxManagerForm->elementStart('div', FALSE, FALSE );
												...
												if ( 
													...
													echo $chatboxManagerForm->element('p', FALSE, FALSE, _e('<span>You have reached the limit of 3 chatboxes. You can unlock this limit visiting this LINK</span> ','chatbox-manager'));
